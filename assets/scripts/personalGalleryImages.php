...
function PersonalGalleryImages(){
	...
	if(isset($_SESSION["user"])){
	    ...
	    $latestimage1 = "";
	    $result = $conn->prepare("SELECT image_id,image_title,image_comment FROM user_image WHERE user_id='$user_id' ORDER BY image_date_added DESC LIMIT 1");
		$result->execute();	
		...
		$latestimage2 = "";
	    $result = $conn->prepare("SELECT image_id,image_title,image_comment FROM user_image WHERE user_id='$user_id' ORDER BY image_date_added DESC LIMIT 1,1");
		$result->execute();		
		...
		$latestimage3 = "";
	    $result = $conn->prepare("SELECT image_id,image_title,image_comment FROM user_image WHERE user_id='$user_id' ORDER BY image_date_added DESC LIMIT 2,1");
		$result->execute();		
		...
		$latestimage4 = "";
	    $result = $conn->prepare("SELECT image_id,image_title,image_comment FROM user_image WHERE user_id='$user_id' ORDER BY image_date_added DESC LIMIT 3,1");
		$result->execute();	
